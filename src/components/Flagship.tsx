...
const STANDING = {
  done: { mark: "✔", tone: "text-ok" },
  next: { mark: "❯", tone: "text-warn" },
  queued: { mark: "·", tone: "text-faint" },
} as const satisfies Record<Standing, { mark: string; tone: string }>;
...
// The project still being built, given the full width. The unfinished rows of
// its plan stay on show: a design argued in the open, with the work landing
// against it, is a stronger claim than a finished-looking card with nothing
// in the repository behind it.
  ...
  return (
    <Lit>
      ...
      <div className="panel-body">
        <h3 className="font-mono text-lg font-medium tracking-tight sm:text-xl">
          {project.name}
        </h3>
        ...
        <p className="border-accent mt-5 max-w-3xl border-l-2 pl-3">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1.45fr)_minmax(0,1fr)] lg:gap-6">
          <div>
              ...
              <ol aria-label={`Build plan for ${project.name}`}>
                {milestones.map((milestone, index) => {
                  ...
                  return (
                    ...
                    >
                      <span aria-hidden="true" className={`plan-mark ${tone}`}>
                        {mark}
                      </span>
